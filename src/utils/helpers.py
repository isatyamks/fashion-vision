...
class URLValidator:
    ...
    def validate_csv(
        self, csv_path: str = str(SHOPIFY_URL_SMALL_CSV)
    ...
    def save_report(
        self, bad_urls: List[str], output_path: str = "outputs/bad_urls.csv"
    ) -> None:
        os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
        pd.DataFrame(bad_urls, columns=["bad_image_url"]).to_csv(
    ...
    def _check(self, url: str) -> Tuple[bool, str]:
        try:
            resp = requests.get(
                url, headers={"User-Agent": "Mozilla/5.0"}, timeout=self.timeout
            )
