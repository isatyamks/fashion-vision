...
class FashionColorClassifier:
    def __init__(
        ...
        self.classifier = pipeline(
            "zero-shot-image-classification", model=model_name, device=device_id
        )
        _, self.labels = load_product_colors()
        self.candidate_labels: List[str] = [
            f"a photo of a {color} garment" for color in self.labels
        ]
    ...
    def predict_color(self, image: Image.Image) -> Tuple[str, float]:
        try:
            ...
            extracted_color: str = (
                best_match["label"].replace("a photo of a ", "").replace(" garment", "")
            )
