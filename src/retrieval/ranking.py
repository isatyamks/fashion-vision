    ...
    def top_matches(self, distances: np.ndarray, indices: np.ndarray, detected_class: str) -> List[Dict[str, Any]]:
        ...
        for pid, emb_sim in product_best_sim.items():
            ...
            if detected_class:
                ...
                if ("top" in det_lower and "top" in prod_type) or                   ("jeans" in det_lower and "jeans" in prod_type) or                   ("dress" in det_lower and "dress" in prod_type) or                   ("shirt" in det_lower and "shirt" in prod_type):
                    cat_match = 1.0
            attr_match = 0.5  
