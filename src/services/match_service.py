...
from src.data.loader import load_product_images, load_product_metadata, load_product_colors
...
class MatchService:
    def __init__(self) -> None:
        ...
        self.color_analyzer: FashionColorClassifier = FashionColorClassifier(device=DEVICE)
        
        main_index_path: str = os.path.join(VECTOR_DB_DIR, "index.faiss")
        main_ids_path: str = os.path.join(VECTOR_DB_DIR, "index_ids.json")
        self.indexer: FaissIndexer = FaissIndexer(index_path=main_index_path, ids_path=main_ids_path, encoder=self.encoder)
    ...
    async def process_match(self, video: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            predicted_color, confidence = self.color_analyzer.predict_color(crops[mid_idx])
            t3: float = time.time()
            ...
            for i in range(len(crops)):
                for j in range(len(indices[i])):
                    ...
                    if idx != -1 and idx < len(self.indexer.product_ids):
                        ...
                        base_id: str = full_id.split('_')[0] if '_' in full_id else full_id
                        
                        visual_score: float = (dist + 1.0) / 2.0
                        color_score: float = 0.0
                        
                        if base_id in self.product_colors_map and predicted_color != "unknown":
            ...
            sorted_matches = sorted(list(product_scores.items()), key=lambda x: x[1], reverse=True)
            t6: float = time.time()
            ...
            for pid, score in sorted_matches[:5]:
                ...
                results.append({
                    "product_id": pid,
                    "confidence": score,
                    "image_url": self.product_images_map.get(pid, "https://via.placeholder.com/400x600?text=Image+Not+Found"),
